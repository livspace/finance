...
yearweek(order_created_at) as week,
project_id,
case when project_id = 288958 then 'Noida'  
...
vmbo.order_state,
sum(ifnull(order_products_wt,0)/1.145) as Revenue,
sum(ifnull(order_discount,0)) as Discounts, 
sum(ifnull(order_products_wt,0)) as 'Amount With Tax',
